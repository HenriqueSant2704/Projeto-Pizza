...
async function carregarCardapio() {
  try {
    const resposta = await fetch(`${window.location.origin}/api/cardapio`);
...
function criarCarrosselCategoria(titulo, listaProdutos, temMontarPizza, containerPrincipal) {
  ...
  listaProdutos.forEach(produto => {
    ...
    card.innerHTML = `
      <img src="${window.location.origin}${produto.imagem}" alt="${produto.descricao}">
      <div class="descricao-mais-pedidos">
...
document.addEventListener("click", async (e) => {
  ...
  if (isAdicionar) {
    ...
    const pFetch = fetch(`${window.location.origin}/api/carrinho/add`, {
      method: "POST",
...
async function atualizarBadgeSemAbrir() {
  try {
    const resposta = await fetch(`${window.location.origin}/api/carrinho/listar`);
...
document.addEventListener("click", async (e) => {
  if (e.target.closest(".remover-iten")) {
    ...
    try {
      const resposta = await fetch(`${window.location.origin}/api/carrinho/deletar/${idCarrinho}`, {
        method: "DELETE"
      });
...
document.addEventListener("click", async (e) => {
  ...
  if (e.target.classList.contains("incrementar")) {
    ...
    try {
      await fetch(`${window.location.origin}/api/carrinho/atualizar/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
